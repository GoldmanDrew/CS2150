...
BinaryNode* BinarySearchTree::insert(BinaryNode* node, const string& x) {
  if(node == NULL) {
    node = new BinaryNode();
    node->value = x;
  }
  else if(node->value.compare(x) > 0) node->left = insert(node->left, x);
...
string BinarySearchTree::pathTo(BinaryNode* node, const string& x) const {
    ...
    if(node->left != NULL && node->value.compare(x) > 0) {
      return node->value + " " +  pathTo(node->left, x);
    }
    if(node->right != NULL && node->value.compare(x) < 0) {
